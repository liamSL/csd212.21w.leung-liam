...
function createSnake(gridPosition) {
  const firstSegment = createSnakeSegment(
    gridPosition,
  ...
  return {
    ...
    isOnPosition: function (position) {
      for (const seg of this.segments) {
        if (isPositionEqual(seg.gridPosition, position)) {
          return true;
        }
      }
      return false;
...
function init() {
  window.addEventListener("beforeunload", function (event) {
    if (
      ...
      event.returnValue =
        "You are in the middle of a game!  Are you sure you want to leave?";
    }
  });
  ...
  document.getElementById("block-size").oninput = (e) => {
    ...
    document.getElementById("block-size-preview").style.width =
      BLOCK_SIZE + "px";
    document.getElementById("block-size-preview").style.height =
      BLOCK_SIZE + "px";
  };
  ...
  document.getElementById("snake-type").addEventListener("change", (e) => {
    ...
    document
      ...
      .setAttribute("placeholder", t + "ss " + t + "SSsss --<");
  });
